...
第 15 章第 04 节增加类型化 AI 文本流和 POST SSE 调试端点：连接断开沿 AbortSignal
取消编排，Socket 缓冲满时等待 `drain`。RTC 仍负责音频，当前也不冒充真实模型 Token 流。
协议与限制见 [`AI_STREAM_PROTOCOL.md`](docs/protocol/AI_STREAM_PROTOCOL.md)。

第 15 章第 05 节增加版本化知识源 Manifest 与进程内治理 Registry：所有访问使用
`tenant_id + source_id` 复合边界，只有当前生效的公开已发布资料可检索；撤回后经过保留期，
并确认 Blob、Chunk、Vector 和 Cache 都清理完成，才能进入 Purged。治理规则见
[`KNOWLEDGE_SOURCE_GOVERNANCE.md`](docs/architecture/KNOWLEDGE_SOURCE_GOVERNANCE.md)。

第 15 章第 06 节增加确定性知识入库 Pipeline：受限 Loader 校验大小、路径与 SHA-256，随后
规范化文本、提取 Markdown Section、生成重叠 Chunk，并附加租户安全的稳定 ID 和标量
Metadata。当前只输出合成 Chunk，不调用 Embedding 或 Chroma。详见
[`KNOWLEDGE_INGESTION_PIPELINE.md`](docs/architecture/KNOWLEDGE_INGESTION_PIPELINE.md)。

第 15 章第 07 节增加版本化 Embedding/Collection 契约与幂等增量索引：稳定 ID 未变化时不再
调用 Embedding，新向量全部 Upsert 成功后才清理旧 ID。默认练习使用确定性向量和内存
Collection；真实 Ark 与 Chroma Cloud 各有独立显式开关。详见
[`KNOWLEDGE_VECTOR_INDEX.md`](docs/architecture/KNOWLEDGE_VECTOR_INDEX.md)。

第 15 章第 08 节把查询规划、Registry Source Allowlist、Dense/关键词两路召回和 RRF 融合
组成候选检索层。两个分支共享服务端 Tenant Scope，未知 Tenant 在 Embedding 前失败关闭；
候选仍不等于可回答证据。详见
[`HYBRID_KNOWLEDGE_RETRIEVAL.md`](docs/architecture/HYBRID_KNOWLEDGE_RETRIEVAL.md)。

第 16 章第 01 节组合现有客服与 RAG 策略，增加固定欢迎语、Grounded Prompt 信任边界、
有界 Session 记忆和确定性安全降级。离线实验只验证应用策略，不调用模型，也不证明模型
一定遵循 Prompt。详见
[`REALTIME_RAG_CONVERSATION_POLICY.md`](docs/architecture/REALTIME_RAG_CONVERSATION_POLICY.md)。

第 16 章第 02 节用一个 `RealtimeRagService` 统一 Mock Voice、调试 HTTP 和调试 SSE 的
幂等、记忆、降级与 Session 清理。默认 Mock 模型和合成知识保持零云费用；Mock 标准事件只
证明接入形状，不代表真实 ASR、TTS 或 RTC 延迟。详见
[`REALTIME_RAG_SERVICE.md`](docs/architecture/REALTIME_RAG_SERVICE.md)。
