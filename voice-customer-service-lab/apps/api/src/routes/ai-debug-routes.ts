...
import type { AiTurnStreamService } from "../ai/ai-turn-stream-service.js";
import { SseEventWriter } from "../ai/sse-event-writer.js";
...
export interface RegisterAiDebugRoutesOptions {
  ...
  readonly streamService: AiTurnStreamService | undefined;
...
export async function registerAiDebugRoutes(
  ...
  routes.post(
    "/api/v1/sessions/:session_id/ai/debug-streams",
    {
      schema: {
        operationId: "createAiDebugStream",
        tags: ["ai-debug"],
        headers: AiDebugTurnHeadersSchema,
        params: SessionParamsSchema,
        body: schemaRef(AiDebugTurnRequestSchema),
        response: {
          200: {
            ...
            content: {
              "text/event-stream": {
                schema: schemaRef(AiDebugStreamResponseSchema),
              },
            },
          },
          400: schemaRef(ApiErrorResponseSchema),
    ...
    async (request, reply) => {
      ...
      const stream = options.streamService.stream({
        tenantId: options.resolveTenantId(session.session_id),
        sessionId: session.session_id,
        text: request.body.text,
        idempotencyKey: request.headers["idempotency-key"],
      ...
      // Start the service before committing SSE headers so synchronous validation/conflicts
      // remain ordinary JSON HTTP errors.
      ...
      try {
        const writer = new SseEventWriter(reply.raw, controller.signal);
        ...
        if (!controller.signal.aborted) {
          request.log.error(
            { event: "ai_sse_stream_failed", error: { name: errorName(error) } },
            "AI SSE stream failed",
          );
          reply.raw.destroy();
        }
      } finally {
