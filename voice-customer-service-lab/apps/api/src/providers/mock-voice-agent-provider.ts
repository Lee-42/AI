  ...
  async submitMockTurn(command: SubmitMockTurnCommand): Promise<SessionCommandResponse> {
    ...
    const events = [
      ...
      this.#event(record, command.correlationId, {
        ...
        payload: { model_route: "mock/rules-v1" },
      }),
      this.#event(record, command.correlationId, {
        event_type: "turn.ai.transcript.delta",
        round_id: roundId,
        response_id: responseId,
        payload: { text_delta: answer, index: 0 },
      }),
      this.#event(record, command.correlationId, {
...
function createMockAnswer(text: string): string {
  if (text.includes("订单")) {
    return "这是模拟订单回复：DEMO-1001 已发货，预计两个工作日内送达。";
  }
  if (text.includes("人工")) {
    return "这是演示环境，我可以记录模拟转人工请求，但当前没有真人坐席接入。";
  }
  return `这是 Mock 客服回复：我收到了“${text}”。当前没有调用真实 AI 模型。`;
}
