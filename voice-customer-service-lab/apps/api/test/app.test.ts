...
describe("HTTP contracts", () => {
  ...
  it("keeps stable OpenAPI operation IDs", async () => {
    ...
    expect(
      schema.paths?.["/api/v1/sessions/{session_id}/ai/debug-streams"]?.post?.operationId,
    ).toBe("createAiDebugStream");
    expect(
      schema.paths?.["/api/v1/sessions/{session_id}/ai/debug-streams"]?.post?.responses?.["200"],
    ).toMatchObject({ content: { "text/event-stream": expect.any(Object) } });
  ...
  it("runs a complete mock session without cloud credentials", async () => {
    ...
    expect(JSON.stringify(turn.events)).toContain("七个自然日");
